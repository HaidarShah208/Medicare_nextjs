...
function Header({ handleToggleSidebar }: HeaderProps) {
  return (
    <div className="border-b border-dark header flex justify-between items-center bg-black-primary  px-4 h-[92px] w-full">
     ...
     <Image src={IMEGES.Logo} alt={"logo"} />
      ...
      <div className="header_icons  flex items-center space-x-4">
        <Image src={IMEGES.Email} className="sm:block hidden" alt={"logo"} />
        <Image src={IMEGES.Notification} className="sm:block hidden" alt={"logo"} />
        <Image src={IMEGES.Logout} className="sm:block hidden" alt={"logo"} />
      </div>
    </div>
  );
}
